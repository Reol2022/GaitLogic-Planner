...
def test_required_routes_are_registered() -> None:
    ...
    assert "GET /api/system-settings" in routes
    ...
    assert "GET /api/plan-imports/{import_id}" in routes
    assert "PATCH /api/plan-imports/{import_id}/items/{item_id}" in routes
