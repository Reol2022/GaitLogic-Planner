...
建议维护者确认是否采用 `AGPL-3.0-only`。在维护者添加正式 `LICENSE` 文件前：

- 不应仅通过修改 README 或文档文字宣称已经完成许可证授权；
- 不应创建“禁止商用但声称开源”的自定义许可证；
- 不应把商标条款写入标准开源许可证正文；
- 任何贡献者提交代码前，应确认自己接受未来维护者明确公布的项目许可证安排。

许可证一经确认，应以仓库根目录的 `LICENSE` 文件为准。

若社区版本未来采用 GNU Affero General Public License v3.0：
