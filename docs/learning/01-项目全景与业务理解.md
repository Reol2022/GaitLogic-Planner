...
| Training Knowledge Corpus | 提供可审计的训练知识文档、来源、Chunk 与 Manifest | 已提交、尚未发布（v0.12.0-A） |
| Embedding/Vector/Retriever | 建立受控索引与检索能力 | 工作区开发中（v0.12.0-B） |
| RAG Agent Tool / RAG Eval | 将检索接入 Coach 并评测引用质量 | 规划中 |
...
v0.11.0 已发布 Coach Agent：八个只读工具、OpenAI-compatible Provider Gateway、结构化输出、确定性 Validator、Fallback、Vue 前端与 32 条离线 Evaluation 案例。它不构成医疗诊断，不写训练计划、训练日志或 Runner State 快照。

### 已提交、尚未发布：v0.12.0-A

提交 `53266aa` 提供 Training Knowledge Corpus Foundation：受控知识来源、Markdown Front Matter、确定性切分、Manifest 和 root hash。该阶段不包含 Embedding、向量存储、Retriever 或 Agent 接入。

### 工作区开发中：v0.12.0-B

当前工作区出现 Embedding Provider、Exact Cosine Vector Store、索引构建 CLI 和 Retriever 的代码及测试，但尚未提交。因此可用于学习和代码审查，不能称为已发布 RAG 能力。

### 未完成或仅规划

- RAG 只读 Tool 接入 Coach Agent。
- RAG Evaluation 与前端知识引用展示。
